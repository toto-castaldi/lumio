-- =============================================================================
-- Migration: Add subfolder filtering to study RPCs
-- Phase 41, Plan 02: Database Foundation
--
-- Adds transparent subfolder_path filtering to get_study_cards_for_session
-- and get_due_card_count. RPC signatures are UNCHANGED -- no new parameters,
-- no changed defaults. The filter is applied based on user_repositories data:
--
--   AND (ur.subfolder_path IS NULL OR c.file_path LIKE ur.subfolder_path || '%')
--
-- When subfolder_path IS NULL (whole-repo subscription): all cards included
--   (backward compatible, identical behavior to before this migration).
-- When subfolder_path IS NOT NULL (subfolder subscription): only cards whose
--   file_path starts with the subfolder_path are included.
-- =============================================================================

-- ============================================================================
-- Function 1: get_study_cards_for_session
-- Subfolder filter added to all 5 user_repositories JOINs
-- ============================================================================
...
COMMENT ON FUNCTION get_due_card_count IS 'Returns count of cards due for review today (timezone-aware) PLUS never-reviewed cards with active questions. When p_limit is NULL, returns full count. When p_limit is set, returns LEAST(total, p_limit) to reflect session-capped count. Filters by subfolder_path when set on user_repositories subscription.';
