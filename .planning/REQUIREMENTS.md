...
- [ ] **TOKEN-01**: New edge function proxies PAT update to Docora `PATCH /api/repositories/{repository_id}/token`
- [ ] **TOKEN-02**: After successful token update, backend optimistically clears repo sync error status
...
- [ ] **TOKUI-01**: User can tap a failed repo to open a bottom-sheet modal with error details
- [ ] **TOKUI-02**: For auth-related errors, user can enter a new PAT in the modal and submit
- [ ] **TOKUI-03**: After successful token update, the repo error state clears immediately in the UI
